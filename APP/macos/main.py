...
case'cards_dealt':logText(p.description||`${tr('marker_card')}: ${localizeCard({label:p.marked_card})} · ${tr('marker_holder')}: ${p.marker_holder_name||''} · ${tr('bottom_cards')}: ${p.bottom_count}`);break;
...
case'bid_result':logText(p.description||`${p.player_name}: ${p.bid?tr('bid')+' '+p.bid:tr('no_bid')}`);break;
case'call_result':logText(p.description||`${p.player_name}: ${yesNoText(p.call,'call_landlord','dont_call')}`);break;
case'rob_result':logText(p.description||`${p.player_name}: ${yesNoText(p.rob,'rob_landlord','dont_rob')}`);break;
case'reveal_result':logText(p.description||`${p.player_name}: ${yesNoText(p.reveal,'reveal','dont_reveal')}`);break;
case'report_result':logText(`${p.player_name}: ${p.report_label||tr('report')}`);break;
case'landlord_assigned':logText(p.description||`${p.player_name}: ${tr('landlord')} · ${cardsText(p.bottom_cards)}`);break;
case'play_turn':logText(p.description||`${tr('seat')} ${p.seat+1} ${p.player_name}: ${tr('play')}${p.is_opening?' · '+tr('round'):''}`);break;
case'new_round':logText(p.description||`${tr('round')}: ${p.leader_name}`);break;
case'no_bidder':logText(p.message||tr('no_bid'));break;
case'redeal':logText(p.description||p.message||tr('redeal_factor'));break;
case'player_empty':logText(`${p.player_name}: ${tr('winner')}`);break;
case'play_action':logText(p.description||(p.action==='play'?`${p.player_name} ${tr('play')}: ${comboText(p.combo,p.combo_display)} -> ${cardsText(p.cards_played)} | ${tr('your_hand')}: ${p.remaining_count}`:`${p.player_name}: ${tr('pass')} | ${tr('your_hand')}: ${p.remaining_count}`));break;
...
`;var backBtn=document.createElement('button');backBtn.textContent=tr('back');backBtn.className='secondary';backBtn.onclick=function(){showView(state.username?'home':'auth')};document.getElementById('actionPanel').appendChild(backBtn)}
