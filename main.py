...
<div class="app">
  ...
  <section id="localView" class="grid hidden">
    <div class="panel stack">
      ...
      <button class="secondary" onclick="exitMatching()" data-i18n="back"></button>
    </div>
    <div id="postGame" class="panel stack hidden">
      <h2 data-i18n="finished"></h2>
      <div id="postGameResult"></div>
      <button class="ok" onclick="continueMatching()" data-i18n="continue_match"></button>
      <button class="secondary" onclick="exitMatching()" data-i18n="exit_match"></button>
    </div>
  </section>
...
function showView(v){for(const id of ['authView','homeView','localView','pvpView','rulesView','gameView'])document.getElementById(id).classList.add('hidden');document.getElementById(v+'View').classList.remove('hidden');if(v==='local')document.getElementById('postGame').classList.add('hidden')}
...
function startMatchmaking(){state.matchType='local';state.matchMode=val('localMode');state.matchKind=val('localMatch');resetGameControls();clearLog();showView('game');document.getElementById('postGame').classList.add('hidden');send({type:'start_local_ai_match',mode:state.matchMode,match_type:state.matchKind})}
function continueMatching(){document.getElementById('postGame').classList.add('hidden');resetGameControls();clearLog();showView('game');send({type:'start_local_ai_match',mode:state.matchMode,match_type:state.matchKind})}
function exitMatching(){state.matchMode=null;document.getElementById('postGame').classList.add('hidden');showView(state.username?'home':'auth')}
...
case'your_hand':case'your_cards':state.cards=p.cards||[];state.selected.clear();renderHand();break;
	case'cards_dealt':logText(`${tr('marker_card')}: ${localizeCard({label:p.marked_card})} · ${tr('marker_holder')}: ${p.marker_holder_name||''} · ${tr('bottom_cards')}: ${p.bottom_count}张`);logHands(p.player_hands);break;
...
case'reveal_result':logText(p.reveal?`${p.player_name} ${tr('reveal')}: ${cardsText(p.hand)} (${p.hand_size}张)`:`${p.player_name}: ${tr('dont_reveal')}`);break;
...
case'play_turn':if(state.matchType!=='demo'){var t=p.last_combo?`${tr('play')} > ${comboText(p.last_combo,p.combo_display)}`:`${tr('play')} (${tr('round')})`;logText(`${p.player_name} ${p.is_opening?t:tr('play')}${p.last_player_name?' | '+p.last_player_name:''}`);}break;
case'new_round':logText(`--- ${tr('round')} ${tr('play')}: ${p.leader_name} ---`);break;
...
case'player_empty':logText(`${p.player_name} ${tr('winner')} — ${tr('your_hand')} 0张`);break;
case'play_action':logText(p.action==='play'?`${p.player_name} ${tr('play')}: ${comboText(p.combo,p.combo_display)} -> ${cardsText(p.cards_played)} (${p.remaining_count}${p.remaining_count>1?'张':'张'})`:`${p.player_name}: ${tr('pass')} (${p.remaining_count}张)`);break;
case'game_over':logText(`=== ${tr('finished')} === ${tr('winner')}: ${p.winner_name||'?'} (${p.winner_role||''}) · ${tr('landlord')}: ${p.landlord_name||'?'}`);showSettlement(p);if(state.matchType==='local'){var pg=document.getElementById('postGame');pg.classList.remove('hidden');document.getElementById('postGameResult').textContent=`${tr('winner')}: ${p.winner_name||'?'} (${p.winner_role||''})`}break;
